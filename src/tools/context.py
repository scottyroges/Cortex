...
def update_initiative_status(
    ...
    """
    Quick update for initiative status.

    Convenience wrapper for updating just the status without changing the initiative name.
    Requires an existing initiative to be set first.

    Args:
        status: New status (e.g., "Phase 2 blocked on API review")
    ...
    logger.info(f"Updating initiative status for repository: {repository}")
    ...
    try:
        ...
        if not existing.get("ids"):
            return json.dumps({
                "error": "No initiative found for this repository",
                "hint": "Use set_initiative to create an initiative first",
            })

        # Get the existing initiative name
        meta = existing["metadatas"][0] if existing.get("metadatas") else {}
        initiative_name = meta.get("initiative_name", "Unknown Initiative")
        ...
        logger.info(f"Initiative status updated for repository '{repository}'")
        ...
        logger.error(f"Update initiative status error: {e}")
        return json.dumps({
            "status": "error",
            "error": str(e),
        })
